...
*/
	...
	public static final boolean eval_IntNumber(String param)
	{ 
	...
	public static final boolean eval_Real(String param)
	{ 
		boolean isReal = true;
		if (param.length() <= 0)
			return false;
	...
	/**
	 * milliseconds为System.currentTimeMillis()
	 * 
	 * @param milliseconds
	 * @param dateformat
	 */
	...
	public static long handleDate(String data, String dateformat, long defaultV)
	{
		if (dateformat == null)
		{
	...
	public static boolean containsOne(String body, boolean isIgnoreCase,
			String... s)
	{
		if (isNull(body))
			return false;
		...
		for (int i = 0; i < s.length; i++)
		{
			if (body.contains(s[i].toLowerCase()))
				return true;
		}
			...
			throws Exception
	{
		...
		{
			throw new Exception("encoding can not empty");
		}
		if (content == null)
		{
		...
		if (size < 0)
		{
			throw new Exception("size value must more than zero");
		}
		...
		while (count < size)
		{
			...
			if (count <= size)
			{
				out.write(content.substring(index, index + 1)
						.getBytes(encoding));
			}
			index++;
		}
	...
	public static String subStringByByte(String str, int len)
	{
		...
		} else
		{
			...
			if (b[len - 1] < 0 && b[len - 2] > 0)
			{ // 判断最后一个字节是否为一个汉字的第一个字节
